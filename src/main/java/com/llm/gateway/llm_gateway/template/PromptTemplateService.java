...
/**
 * Loads per-provider .st prompt templates and renders them with caller-supplied variables.
 * Falls back to raw systemPrompt when no templateVars are provided (backward compat).
 */
@Service
public class PromptTemplateService {
    ...
    /**
     * Returns the resolved system prompt for the given provider and request.
     * Priority:
     *  1. If templateVars present → render per-provider .st template with merged vars.
     *  2. If systemPrompt set (no templateVars) → return it unchanged (backward compat).
     *  3. Otherwise → render per-provider .st template with defaults only.
     */
    ...
    /**
     * Renders the assistant-starter.st template with the supplied vars.
     * Returns null if the request carries no assistantMessage and no vars with a "starter" key.
     */

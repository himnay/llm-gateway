 ...
 *
 * <p>{@link #execute(LlmRequest)} fixes the invariant algorithm (resolve model, render system
 * prompt, POST JSON, parse, never throw — errors become {@link LlmResponse#getError()}), while
 * subclasses supply only the provider-specific hooks: endpoint, headers, request payload and
 * response parsing. Used by the providers that talk to plain REST APIs (Google Gemini, Cohere,
 * HuggingFace); the Spring AI providers (OpenAI, Anthropic, Ollama) go through ChatClient instead.
 */
public abstract class AbstractRestLlmService implements LlmServiceProvider {
  ...
  protected AbstractRestLlmService(
      WebClient webClient,
  ...
  protected abstract Map<String, Object> buildPayload(
      LlmRequest request, String model, String systemText);

  /**
   * Extracts content/usage from the provider's JSON. Return a partially-populated builder —
   * provider, model and timestamp are filled in by the template.
   */

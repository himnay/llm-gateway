...
/**
 * Google Gemini integration via the Generative Language REST API (v1beta). Endpoint: POST
 * https://generativelanguage.googleapis.com/v1beta/models/{model}:generateContent Docs:
 * https://ai.google.dev/api/generate-content
 */
@Service
@ConditionalOnProperty(
    prefix = "llm.providers.google",
    name = "enabled",
    havingValue = "true",
    matchIfMissing = true)
public class GoogleGeminiService extends AbstractRestLlmService {
  ...
  protected Map<String, Object> buildPayload(LlmRequest request, String model, String systemText) {
    ...
    if (systemText != null && !systemText.isBlank()) {
      payload.put("systemInstruction", Map.of("parts", List.of(Map.of("text", systemText))));
    }

    payload.put(
        "contents",
        List.of(Map.of("role", "user", "parts", List.of(Map.of("text", request.getPrompt())))));
  ...
  protected LlmResponse.LlmResponseBuilder parseResponse(JsonNode root, String model) {
    ...
    return LlmResponse.builder()
        .content(text)
        .promptTokens(
            usage.has("promptTokenCount") ? usage.path("promptTokenCount").intValue() : null)
        .completionTokens(
            usage.has("candidatesTokenCount")
                ? usage.path("candidatesTokenCount").intValue()
                : null)
        .totalTokens(
            usage.has("totalTokenCount") ? usage.path("totalTokenCount").intValue() : null);
  }
}

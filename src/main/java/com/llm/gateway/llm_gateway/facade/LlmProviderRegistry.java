...
/**
 * Provider registry (GoF <b>Factory Method</b> / Registry) — the single place where
 * {@link LlmServiceProvider} <b>Strategy</b> implementations are discovered and
 * resolved by name.
 *
 * <p>Spring auto-discovers every {@code @Service} implementing
 * {@link LlmServiceProvider}; adding a provider requires no change here or in the
 * facade (Open/Closed Principle).</p>
 */
@Slf4j
@Component
public class LlmProviderRegistry {
    ...
    void logRegisteredProviders() {
        log.info("LLM provider registry initialised with {} providers: {}", providers.size(), providers.keySet());
    }

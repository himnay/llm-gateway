 ...
 *       .call().content();
 *
 * The LLM decides autonomously when to invoke each tool based on the user prompt.
 */
@Component
public class GatewayTools {

    @Tool(description = "Get the current date and time in ISO-8601 format for a given timezone. "
    ...
    @Tool(description = "Get information about a specific LLM provider supported by this gateway, "
    ...
    @Tool(description = "Calculate the estimated cost in USD for a given number of input and output tokens "
    ...
    public String estimateTokenCost(String model, int inputTokens, int outputTokens) {
        ...
        switch (model.toLowerCase()) {
            case "gpt-4o"             -> { inputCost = 0.005;  outputCost = 0.015; }
            case "gpt-4-turbo"        -> { inputCost = 0.01;   outputCost = 0.03; }
            case "gpt-3.5-turbo"      -> { inputCost = 0.0005; outputCost = 0.0015; }
            case "claude-3-5-sonnet-20241022" -> { inputCost = 0.003; outputCost = 0.015; }
            case "claude-3-opus-20240229"     -> { inputCost = 0.015; outputCost = 0.075; }
            default                   -> { inputCost = 0.001;  outputCost = 0.002; }
        }
        double totalCost = (inputTokens / 1000.0 * inputCost) + (outputTokens / 1000.0 * outputCost);
        return String.format("Estimated cost for model '%s': $%.6f USD "

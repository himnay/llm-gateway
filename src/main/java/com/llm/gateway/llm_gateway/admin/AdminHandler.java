 ...
 *
 * <p>Routes (all under /llm/admin/, require auth): GET /admin/keys — list all keys (names, status;
 * no hashes) POST /admin/keys — create a new key (returns raw key ONCE) PATCH /admin/keys/{id} —
 * enable / disable a key or set expiry DELETE /admin/keys/{id} — permanently delete a key
 */
@Slf4j
@Component
...
public class AdminHandler {
  ...
  /**
   * Creates a new API key. Body: {@code {"name":"...", "client_id":"...", "expires_at":"ISO-8601 or
   * null"}}. Returns the raw key exactly once — it is never stored and cannot be retrieved again.
   */
  ...
  /**
   * Enables, disables, or sets expiry on a key. Body: {@code {"enabled": true|false, "expires_at":
   * "ISO-8601 or null"}}.
   */
        ...
        .flatMap(
            body -> {
              ...
              return db.sql(
                  ...
                  .bind(
                      "expiresAt",
                      expiresAt != null ? expiresAt.toString() : Instant.now().toString())
                  .fetch()
                  .one()
                  .flatMap(
                      row -> {

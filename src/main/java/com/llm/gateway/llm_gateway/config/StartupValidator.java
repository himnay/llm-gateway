...
/**
 * Emits startup warnings when enabled LLM providers have missing or placeholder API keys.
 * Does not fail startup — providers without keys will error at request time instead.
 */
@Slf4j
@Component
...
public class StartupValidator {
    ...
    public void validateProviderKeys() {
        ...
        providerProperties.getProviders().forEach((provider, config) -> {
            if (!config.isEnabled()) return;

            String key = config.getApiKey();
            if (!StringUtils.hasText(key)) {
                warnings.add(String.format("Provider '%s' is enabled but has no API key configured", provider));
            } else if (key.startsWith(PLACEHOLDER_PREFIX)) {
                warnings.add(String.format("Provider '%s' is using a placeholder API key — real requests will fail", provider));
            }
        });

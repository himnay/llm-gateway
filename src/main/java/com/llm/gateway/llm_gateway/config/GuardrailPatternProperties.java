...
/**
 * Externalised guardrail pattern catalogue — the single, tunable source of truth for
 * every regex / keyword list used by the gateway's guardrails.
 *
 * <p>Bound from {@code llm.guardrails.patterns.*}. Patterns can be added, removed, or
 * edited entirely in YAML / environment configuration <b>without code changes</b>. The
 * fields below carry sensible, fail-safe defaults so protection is never accidentally
 * disabled when the configuration is absent; any value present in configuration
 * <b>replaces</b> the corresponding default list/map.</p>
 *
 * <p>Consumed by {@code SensitiveDataRedactor}, {@code PromptSanitizer} and
 * {@code ToxicityFilterAdvisor}.</p>
 */
@Component
...
public class GuardrailPatternProperties {
    ...
    /**
     * Sensitive-data (PII + secrets) detectors: {@code type-name -> regex}.
     * The type name becomes the redaction placeholder, e.g. {@code email -> [EMAIL]}.
     * Insertion order matters — more specific patterns should come first.
     */
    ...
    private static Map<String, String> defaultSensitiveData() {
        ...
        m.put("private-key",  "-----BEGIN (?:RSA |EC |OPENSSH |PGP )?PRIVATE KEY-----[\\s\\S]*?-----END (?:RSA |EC |OPENSSH |PGP )?PRIVATE KEY-----");
        ...
        m.put("credit-card",  "\\b(?:4[0-9]{12}(?:[0-9]{3})?|5[1-5][0-9]{14}|3[47][0-9]{13}|6(?:011|5[0-9]{2})[0-9]{12})\\b");
        m.put("ssn",          "\\b\\d{3}[-\\s]\\d{2}[-\\s]\\d{4}\\b");
        m.put("iban",         "\\b[A-Z]{2}\\d{2}[A-Z0-9]{4}\\d{7}(?:[A-Z0-9]?){0,16}\\b");
        m.put("ip-address",   "\\b(?:(?:25[0-5]|2[0-4]\\d|[01]?\\d\\d?)\\.){3}(?:25[0-5]|2[0-4]\\d|[01]?\\d\\d?)\\b");

 ...
 *
 * <p>Generates a vector embedding for the given text using OpenAI's embedding model
 * (text-embedding-3-small by default). Both Ollama and OpenAI starters register EmbeddingModel
 * beans; the OpenAI one is explicitly selected here.
 */
@Slf4j
@Component
        ...
        .flatMap(
            request -> {
              if (request.getText() == null || request.getText().isBlank()) {
                return ServerResponse.badRequest().bodyValue(Map.of("error", "'text' is required"));
              }
        ...
        .onErrorResume(
            ex -> {
              ...
              return ServerResponse.status(502)
                  .contentType(MediaType.APPLICATION_JSON)
                  .bodyValue(
                      com.llm.gateway.llm_gateway.embed.EmbeddingResponse.builder()
                          .error(ex.getMessage())
                          .requestId(cid)
                          .timestamp(System.currentTimeMillis())
                          .build());

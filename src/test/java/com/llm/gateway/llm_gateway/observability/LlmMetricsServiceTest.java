...
/**
 * Unit tests for {@link LlmMetricsService} — verifies that the new error,
 * failover, token-usage and guardrail-rejection counters are correctly
 * registered and incremented in the {@link MeterRegistry}.
 */
class LlmMetricsServiceTest {
    ...
    void recordProviderError_incrementsCounter() {
        ...
        Counter openaiTimeout = registry.find("llm.provider.error.total")
                .tag("provider", "openai")
        ...
        Counter anthropicAuth = registry.find("llm.provider.error.total")
    ...
    void recordFailover_incrementsCounter() {
        ...
        Counter openaiToAnthropic = registry.find("llm.provider.failover.total")
                .tag("from", "openai")
                .tag("to", "anthropic")
                .counter();
        ...
        Counter anthropicToOllama = registry.find("llm.provider.failover.total")
                .tag("from", "anthropic")
                .tag("to", "ollama")
                .counter();
    ...
    void recordTokenUsage_registersPromptAndCompletionCounters() {
        ...
        Counter promptCounter = registry.find("llm.token.usage")
                .tag("provider", "openai")
                .tag("type", "prompt")
                .counter();
        ...
        Counter completionCounter = registry.find("llm.token.usage")
                .tag("provider", "openai")
                .tag("type", "completion")
                .counter();
    ...
    void recordGuardrailRejection_incrementsCounter() {
        ...
        Counter injectionCounter = registry.find("llm.guardrail.rejection.total")
                .tag("reason", "PROMPT_INJECTION")
                .counter();
        assertThat(injectionCounter).isNotNull();
        assertThat(injectionCounter.count()).isEqualTo(2.0);

        Counter piiCounter = registry.find("llm.guardrail.rejection.total")
                .tag("reason", "PII_DETECTED")
                .counter();

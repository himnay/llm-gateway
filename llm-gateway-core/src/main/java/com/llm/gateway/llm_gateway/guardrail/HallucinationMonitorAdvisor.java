...
/**
 * OUTPUT guardrail — async heuristic hallucination detection. Zero latency impact: the check runs
 * fire-and-forget after the response is returned.
 *
 ...
 * precise factual grounding, consider retrieval-augmented generation (RAG) or dedicated
...
public class HallucinationMonitorAdvisor implements BaseAdvisor {
  ...
  private void analyse(String text, String provider) {
    ...
    if (score >= THRESHOLD) {
      log.warn(
          "GUARDRAIL | HALLUCINATION_SUSPECTED | provider={} | score={} | uncertainty={} | signals={}",
          provider,
          score,
          uncertainty,
          signals);

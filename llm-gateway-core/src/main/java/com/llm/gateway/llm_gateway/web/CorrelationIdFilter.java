...
/**
 * WebFlux filter that propagates a per-request correlation ID through the entire request/response
 * lifecycle.
 *
 * <ul>
 *   <li>Reads {@code X-Request-ID} from the inbound request header.
 *   <li>Generates a random UUID if the header is absent or blank.
 *   <li>Stores the ID in {@link ServerWebExchange} attributes under key {@code "requestId"} so that
 *       handlers can retrieve it without touching the reactive context.
 *   <li>Publishes the ID into the Reactor {@link Context} under key {@code "requestId"} so that
 *       downstream reactive chains can read it via {@code Mono.deferContextual} / {@code
 *       contextWrite}.
 *   <li>Returns the ID in the {@code X-Request-ID} response header so that clients can correlate
 *       their requests with gateway logs.
 *   <li>Sets the ID in SLF4J {@code MDC} for the duration of the filter chain using a {@code
 *       doFinally} hook to clean up.
 * </ul>
 *

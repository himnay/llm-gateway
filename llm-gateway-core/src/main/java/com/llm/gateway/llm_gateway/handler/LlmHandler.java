...
public class LlmHandler {
            ...
            request -> {
              String provider =
                  request.getProvider() != null
                      ? request.getProvider().key()
                      : LlmProvider.OPENAI.key();
              log.info(
                  "HANDLER | query | provider={} | session={} | cid={}",
                  provider,
  ...
  public Mono<ServerResponse> perProviderChat(ServerRequest req) {
    ...
    return req.bodyToMono(LlmRequest.class)
        ...
        .flatMap(
            request ->
                Mono.fromCallable(() -> facade.execute(provider, request))
                    .subscribeOn(Schedulers.boundedElastic())
                    .timeout(Duration.ofSeconds(timeoutSeconds)))
        .flatMap(resp -> ok(resp, cid))
        .onErrorResume(this::errorResponse);
            ...
            request -> {
              ...
              String provider =
                  request.getProvider() != null
                      ...
                      : LlmProvider.OPENAI.key();
              log.info(
                  "HANDLER | chat | provider={} | session={} | cid={}",
                  provider,
                  request.getSessionId(),
                  cid);
  ...
  public Mono<ServerResponse> extractStructured(ServerRequest req) {
    ...
    return req.bodyToMono(LlmRequest.class)
        ...
        .flatMap(
            request ->
                Mono.fromCallable(() -> facade.executeStructured(request))
                    .subscribeOn(Schedulers.boundedElastic())
                    .timeout(Duration.ofSeconds(timeoutSeconds)))
        .flatMap(resp -> ok(resp, cid))
        .onErrorResume(this::errorResponse);
  ...
  public Mono<ServerResponse> models(ServerRequest req) {
    ...
    Map<String, List<String>> alternatives =
        Map.of(
            ...
            "huggingface",
                List.of("mistralai/Mistral-7B-Instruct-v0.1", "meta-llama/Llama-2-7b-chat"),
  ...
  private static void validate(LlmRequest request) {
    ...
    if (request.getPrompt().length() > 10_000) {
      throw new InvalidRequestException(
          "'prompt' exceeds the maximum allowed length of 10,000 characters");
    }
  }
      ...
      return ServerResponse.status(HttpStatus.GATEWAY_TIMEOUT)
          .bodyValue(
              Map.of(
                  ...
                  "Request timed out",
                  "message",
                  "Provider did not respond within " + timeoutSeconds + "s"));
    }

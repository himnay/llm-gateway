...
/**
 * HuggingFace Serverless Inference API — OpenAI-compatible chat endpoint. Endpoint: POST
 * https://api-inference.huggingface.co/v1/chat/completions Docs:
 * https://huggingface.co/docs/api-inference/tasks/chat-completion
 *
 * <p>This endpoint is compatible with any model that supports the chat completion task on the
 * HuggingFace Hub (e.g. Mistral, Llama, Qwen, Phi).
 */
@Service
@ConditionalOnProperty(
    prefix = "llm.providers.huggingface",
    name = "enabled",
    havingValue = "true",
    matchIfMissing = true)
public class HuggingFaceService extends AbstractRestLlmService {
  ...
  protected LlmResponse.LlmResponseBuilder parseResponse(JsonNode root, String model) {
    ...
    return LlmResponse.builder()
        ...
        .completionTokens(
            usage.has("completion_tokens") ? usage.path("completion_tokens").intValue() : null)

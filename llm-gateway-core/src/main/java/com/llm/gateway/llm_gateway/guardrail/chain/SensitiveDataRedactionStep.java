...
/**
 * Chain step 200 — provider-agnostic PII/secret redaction of the inbound prompt. Runs for EVERY
 * provider — including the custom REST ones (Google, Cohere, HuggingFace) that bypass the Spring AI
 * advisor chain — so no PII or secret is ever forwarded to an LLM, cached, or logged.
 */
@Slf4j
@Component
...
public class SensitiveDataRedactionStep implements GuardrailStep {
  ...
  public void apply(GuardrailContext context) {
    ...
    if (redaction.redacted()) {
      ...
      metricsService.recordSensitiveDataRedaction(
          context.getProvider(), "inbound", redaction.types());
      log.info(
          "SECURITY | Sensitive data redacted from prompt | requestId={} | types={}",

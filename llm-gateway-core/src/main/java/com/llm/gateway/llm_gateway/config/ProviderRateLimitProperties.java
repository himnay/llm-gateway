 ...
 *
 * <p>Bound from {@code gateway.provider-rate-limits.limits} in application.yaml. Provides a simple
 * in-memory sliding minute-window advisory check backed by per-provider counters. If a provider has
 * no entry in the limits map, calls are always allowed through.
 *
 * <pre>
 * gateway:
...
public class ProviderRateLimitProperties {
  ...
  /**
   * Provider name (lower-case) -> per-provider limit settings. Spring Boot populates this from
   * {@code gateway.provider-rate-limits.limits.*}.
   */
  ...
  /**
   * Advisory request-rate check: increments the per-minute counter for the provider and returns
   * {@code true} if the limit has NOT been exceeded. When no limit is configured for the provider,
  ...
  public boolean checkAndIncrementRequest(String provider) {
    ...
    synchronized (slot) {
      ...
      if (!withinLimit) {
        log.warn(
            "PROVIDER_RATE_LIMIT | requests/min exceeded | provider={} | count={} | limit={}",
            provider,
            slot[1],
  ...
  /**
   * Advisory token-rate check: adds {@code tokenCount} to the per-minute token counter and returns
   * {@code true} if the limit has NOT been exceeded. When no limit is configured for the provider,
   * always returns {@code true}.
   */
  public boolean checkAndIncrementTokens(String provider, int tokenCount) {
    ...
    synchronized (slot) {
      ...
      if (!withinLimit) {
        log.warn(
            "PROVIDER_RATE_LIMIT | tokens/min exceeded | provider={} | tokens={} | limit={}",
            provider,
            slot[1],
            limit.getTokensPerMinute());
      }
      return withinLimit;
    }
  }

...
/**
 * INPUT + OUTPUT guardrail — PII / sensitive-data detection and redaction for the Spring AI
 * ChatClient providers (OpenAI, Anthropic, Ollama).
 *
 * <p>INPUT : replaces detected PII/secrets with typed placeholders before the LLM sees it. OUTPUT :
 * async scan to flag any leakage in the response (non-blocking).
 *
 * <p>Detection logic lives in {@link SensitiveDataRedactor} (single source of truth); the gateway
 * facade applies the same redactor to <b>all</b> providers, so this advisor is defence-in-depth for
 * the ChatClient path.
 */
@Slf4j
@Component
...
public class PiiRedactionAdvisor implements BaseAdvisor {
  ...
  public ChatClientRequest before(ChatClientRequest request, AdvisorChain chain) {
    ...
    return request
        .mutate()
        .prompt(request.prompt().augmentUserMessage(m -> m.mutate().text(result.text()).build()))
        .build();
  }

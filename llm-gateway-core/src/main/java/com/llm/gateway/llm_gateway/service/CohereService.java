...
/**
 * Cohere Command R integration using the v2 chat API. Endpoint: POST https://api.cohere.com/v2/chat
 * Docs: https://docs.cohere.com/reference/chat
 */
@Service
@ConditionalOnProperty(
    prefix = "llm.providers.cohere",
    name = "enabled",
    havingValue = "true",
    matchIfMissing = true)
public class CohereService extends AbstractRestLlmService {
  ...
  protected LlmResponse.LlmResponseBuilder parseResponse(JsonNode root, String model) {
    ...
    return LlmResponse.builder()
        ...
        .completionTokens(
            usage.has("output_tokens") ? usage.path("output_tokens").intValue() : null);
  }
}

...
/**
 * OUTPUT guardrail — response format and quality validation. All checks are async (fire-and-forget)
 * to add zero latency to the response path.
 */
@Slf4j
@Component
...
public class ResponseFormatAdvisor implements BaseAdvisor {
  ...
  private static final List<String> REFUSAL_PATTERNS =
      List.of(
          "i cannot",
          "i can't",
          "i am unable to",
  ...
  private static final List<String> TRUNCATION_SIGNALS =
      List.of("...", "…", "[continued]", "[truncated]", "to be continued");
  ...
  private void validate(String text, String provider) {
    ...
    if (length < minLength) {
      log.warn(
          "GUARDRAIL | RESPONSE_TOO_SHORT | provider={} | length={} | min={}",
          provider,
          length,
          minLength);
    ...
    if (length > maxLength) {
      log.warn(
          "GUARDRAIL | RESPONSE_TOO_LONG | provider={} | length={} | max={}",
          provider,
          length,
          maxLength);
    ...
    TRUNCATION_SIGNALS.stream()
        ...
        .ifPresent(
            s ->
                log.warn(
                    "GUARDRAIL | TRUNCATION_SUSPECTED | provider={} | signal={}", provider, s));
  }
}
